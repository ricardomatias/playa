...
import { FLATS, SHARPS } from '../core/constants';
 ...
 */
const assignOctaves = (notes, octaves = [ -1, 11 ], type = 'scale') => {
	const firstNote = notes[0];
	const chromaticNotes = firstNote.isFlat() ? FLATS : SHARPS;
	const map = [];
	...
	allMidiNotes.forEach((note) => {
		...
		if (chromaticNote === notes[notesIndex].n) {
			const mappedNote = new Note(chromaticNote, midi);
