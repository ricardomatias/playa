...
class Key extends Scale {
	 ...
	 * @param {Object} [opts = {}]
	 * @param {Array<Number>} [opts.chordStructure = Chord.SEVENTH ] - f.ex: Chord.SIXTH
	 */
	...
	*/
	...
	modulate(direction, interval = '5P') {
		...
		if (direction === Key.MOD_UP) {
			root = distance.transposeUp(R.head(notes).note, interval);
		} else
		if (direction === Key.MOD_DOWN) {
			root = distance.transposeDown(R.head(notes).note, interval);
		}
