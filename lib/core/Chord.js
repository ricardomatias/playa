...
import { whilst, natural, PlayaError } from '../utils';
...
class Chord extends HarmonyBase {
	...
	*/
	noteAt(interval) {
		let defaultIntervals = this.structure[1];

		if (Array.isArray(defaultIntervals)) {
			defaultIntervals = this.type === CHORDS['SUS2'] ? defaultIntervals[0] : defaultIntervals[1];
		}
