...
import { assignOctaves, whilst, natural } from '../utils';
import { distance, shuffle, rotate, choose, randomInt } from '../tools';
...
class Chord extends HarmonyBase {
	...
	*/
	...
	assignOctaves(octaves) {
		...
		const notes = R.uniqBy(R.prop('note'), this._notes);

		this._notes = assignOctaves(notes, this.octaves, { type: 'chord', hasFlats: this.hasFlats });
	}

	invert(nrOfTimes) {
		this._notes = rotate(this._notes, nrOfTimes || randomInt(R.length(this._notes)));
	...
	_createChordByStructure(structure = Chord.TRIAD) {
		...
		let intervals = defaultIntervals.split(' ');

		const scale = new Scale(root, this._type, { noteType: NoteType.NOTE });
		...
		if (chordIntervals.length !== nrOfNotes) {
			let fromDiffChords = this._createFromDiffChords(structure, scale, scaleIntervals, nrOfNotes);
	...
	_createFromDiffChords(structure, scale, scaleIntervals, nrOfNotes) {
		...
		for (let index = 0; index < types.length; index++) {
			...
			let fromScale = this._createFromScale(scale, scaleIntervals, nrOfNotes, chord.split(' '));
