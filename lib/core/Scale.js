...
class Scale extends HarmonyBase {
	 ...
	 */
	noteAt(interval) {
		const intervals = this._type.split(' ').map((interv) => parseInt(interv.replace(/[^\d]/, '')));

		const noteIndex = intervals.indexOf(interval);

		if (noteIndex === -1) {
			throw new PlayaError('Scale', `[${this._type}] structure doesn't contain interval: ${interval}`);
		}
	...
	_createScale() {
		...
		const rootEnh = rootNote.e ? new Note(rootNote.e) : null;
