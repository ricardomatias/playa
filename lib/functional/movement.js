...
function modulate(key, events) {
	...
	events.push({ key: key.root, interval, type: 'modulate', scaleName: Key.NAMES[key.type] });
	...
	let timeline = euclidean.create(MAX_TURNS, turns)
		...
		.map((event) => {
			...
			if (modProb > random()) {
				...
				events.push({ key: mainKey, type: 'modulateMode', scaleName: Key.NAMES[event.key.scale] });
			}
		...
		.map((event, index, timeline) => {
			...
			if (timeline[nextQuarter]) {
				event.dur = `${(timeline[nextQuarter].quarter - event.quarter) * QUARTER}i`;
			} else {
				event.dur = `${ticks - (event.quarter * QUARTER)}i`;
			}
