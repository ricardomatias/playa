...
const { ticksToTransport, mapDurations } = Time;
const { random, randomInt } = Random;
const { generateFreeRhythm, generateTurnRhythm } = Rhythm;
...
function createChordProgression(
	...
	const progression = [];
	const chords = [];
	const structuresProb = distribute.decreasing(structures);
	let chosenStyle;

	for (let index = 0; index < timeline.length; index++) {
		const { time, dur, key: { scale, root }} = timeline[index];
		const structure = roll(structures, structuresProb, random);
