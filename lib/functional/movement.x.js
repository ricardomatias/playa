...
import { transportToTicks, choose, euclidean, random, distribute, roll } from '../tools';
...
function modulate(key, events) {
	...
	// since 4P/5P up or down only leads to ONE accident
	...
	const interval = roll(Key.MOD_INTERVALS, probabilities);
 ...
 * @return {Array<Object>}
 */
function movement(key, length, { timeSignatures, turns, modProb, repeats }) {
	const events = [];
