...
// - f.ex: `motif(A major, 2 bars)`
// - Generates a rhythm
// - Patterns of time
// - Knows which notes to play
//   - Receives them
//   - Fetches them
// - Generate different combinations of sound/silence with a given pitch class
 ...
 * @return {Array<Motif>}
 */
function motif(notes, length = '1:0:0', rhythmAlgo = normal) {
	// What's a rhythm?
	...
	return pattern.map((event, index) => {
		let prevTime = pattern[index - 1] ? toNumber(pattern[index - 1].time) : null;

		if (index === 0) {
			// do nothing
		} else
		if (index === 1) {
			event.time = `${TICKS.get(event.dur)}i`;
		} else {
