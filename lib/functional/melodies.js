...
function createMelodies(
	...
	for (let index = 0; index < timeline.length; index++) {
		...
		if (rhythmType === TURN_RHYTHM) {
			const probabilities = distribute.descending(minNoteValues.length);
			const minNoteValue = roll(minNoteValues, probabilities);
			const turns = randomInt(Math.floor(dur / TICKS.get(`${minNoteValue}t`)), 2);
