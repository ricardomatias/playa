...
const { mapDurations, ticksToTransport } = Time;
...
function createMelodies(
	...
	for (let index = 0; index < timeline.length; index++) {
		...
		// * PHASE: DEJA VU
		// **************************************************************************
		if (melodies.length && R.gt(dejaVuChance, Random.random())) {
			if (melodiesMap[dur]) {
		...
		if (rhythmType === TURN_RHYTHM) {
			...
			const minNoteValue = roll(minNoteValues, probabilities, Random.random);
		...
		motif = R.map((event) => {
			if (R.gt(restProb, Random.random())) {
				return {
					time: event.time,
					dur: event.dur,
					midi: 0,
