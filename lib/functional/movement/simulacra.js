...
function movementX(key, turns = DEFAULT_TURNS, length, {
	...
	const events = [];
	const ticks = Time.bbsToTicks(length);
	...
	if (rhythm === TURN) {
		timeline = Rhythm.turn(length, turnsNumber, {
			minNoteValue: 8,
			combSorting: {
				diverseFirst: true,
			},
	...
	timeline = timeline.map((event) => {
		...
		if (turn.type === FREE) {
			...
			interval = FREE; // * a FREE turn type must have a FREE interval
		}

		if (!interval && turn.interval !== FREE) {
