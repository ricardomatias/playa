 ...
 */
export default (length, turns, { minNoteValue = 8, combSorting = {}, debug = false } = {}) => {
	let totalRhythmDuration = typeof length === 'number' ? length : transportToTicks(length);

	const grouping = drawGroupingCombination(turns, combSorting);

	const MAX_TURNS = totalRhythmDuration / TICKS.get(`${minNoteValue}t`);
	...
	// we need to add 1 to the grid length in order for the spacing
	...
	for (let index = 0; index < grouping.length; index++) {
		...
		for (let hitsIndex = 0; hitsIndex < nrOfHits; hitsIndex++) {
			...
			grid[gridIndex].dur = TICKS.get(dur);
