 ...
 * @return {Number}
 */
export const transportToTicks = (time, { positionMode = false, ppq = QUARTER } = {}) => {
	try {
		const bar = ppq * 4;
 ...
 * @return {String}
 */
export const ticksToTransport = (ticks, { ppq = QUARTER, timeSignature = [ 4, 4 ], positionMode = false } = {}) => {
 ...
 * @return {Array<NoteEvent>} pattern
 */
export const expandDuration = (pattern, mapToTicks = false) => {
	if (!Array.isArray(pattern)) return [];

	if (typeof pattern[0] === 'string') {
		pattern = pattern.map((dur) => ({ time: 0, dur }));
	}
